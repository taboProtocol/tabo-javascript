...
const TaboBalance = require("./model/TaboBalance");
const TaboReserveInfo = require("./model/TaboReserveInfo");
...
class MoneroWalletFull extends MoneroWalletKeys {
  ...
  static async openWallet(configOrPath, password, networkType, daemonUriOrConnection, proxyToWorker, fs) {
    ...
    if (typeof configOrPath === "object") {
      ...
      if (
        ...
        throw new MoneroError("Can specify config object or params but not both when opening WASM wallet");
    } else {
      config = new MoneroWalletConfig()
        .setPath(configOrPath)
  ...
  static async createWallet(config) {
    ...
    if (
      config.getMnemonic() !== undefined &&
      (config.getPrimaryAddress() !== undefined ||
        config.getPrivateViewKey() !== undefined ||
        config.getPrivateSpendKey() !== undefined)
    ) {
    ...
    if (config.getMnemonic() !== undefined) {
      if (config.getLanguage() !== undefined)
        throw new MoneroError("Cannot provide language when creating wallet from mnemonic");
  ...
  static async _createWalletFromMnemonic(config) {
    ...
    let wallet = await module.queueTask(async function () {
      return new Promise(function (resolve, reject) {
        ...
        LibraryUtils.setRejectUnauthorizedFn(rejectUnauthorizedFnId, function () {
  ...
  static async _createWalletRandom(config) {
    ...
    let wallet = await module.queueTask(async function () {
      return new Promise(function (resolve, reject) {
        ...
        console.log("Creating full wallet with config: " + JSON.stringify(config.toJson()));
        // create wallet in wasm and invoke callback when done
        module.create_full_wallet(JSON.stringify(config.toJson()), rejectUnauthorizedFnId, callbackFn);
      });
    });
  ...
  async sync(listenerOrStartHeight, startHeight, allowConcurrentCalls) {
    ...
    // normalize params
    startHeight =
      listenerOrStartHeight === undefined || listenerOrStartHeight instanceof MoneroWalletListener
        ? startHeight
  ...
  async getReserveInfo() {
    ...
    return that._module.queueTask(async function () {
      ...
      return new Promise(function (resolve, reject) {
        let callbackFn = function (resp) {
          const reserveInfo = new TaboReserveInfo(JSON.parse(GenUtils.stringifyBIs(resp)).reserve_info);
          resolve(reserveInfo);
        };
        that._module.get_reserve_info(that._cppAddress, callbackFn);
      });
    });
  }

  async getBalance(accountIdx, subaddressIdx, asset_type) {
    let that = this;
    return that._module.queueTask(async function () {
      that._assertNotClosed();
      ...
      if (accountIdx === undefined) {
        ...
        balanceStr = that._module.get_balance_account(that._cppAddress, asset_type, accountIdx);
      } else {
        balanceStr = that._module.get_balance_subaddress(that._cppAddress, asset_type, accountIdx, subaddressIdx);
      }

      // parse json string to BigInteger
      return new TaboBalance(JSON.parse(GenUtils.stringifyBIs(balanceStr)).balance, asset_type);
    });
  }

  async getUnlockedBalance(accountIdx, subaddressIdx, asset_type) {
    let that = this;
    return that._module.queueTask(async function () {
      that._assertNotClosed();
      ...
      if (accountIdx === undefined) {
        ...
        unlockedBalanceStr = that._module.get_unlocked_balance_account(that._cppAddress, asset_type, accountIdx);
      } else {
      ...
      // parse json string to BigInteger
      return new TaboBalance(JSON.parse(GenUtils.stringifyBIs(unlockedBalanceStr)).unlockedBalance, asset_type);
    });
  }
  ...
  async getSubaddresses(accountIdx, subaddressIndices) {
    let args = {
      ...
      subaddressIndices: subaddressIndices === undefined ? [] : GenUtils.listify(subaddressIndices),
    };
    let that = this;
    return that._module.queueTask(async function () {
      that._assertNotClosed();
      let subaddressesJson = JSON.parse(
        GenUtils.stringifyBIs(that._module.get_subaddresses(that._cppAddress, JSON.stringify(args))),
      ).subaddresses;
      let subaddresses = [];
    ...
    return that._module.queueTask(async function () {
      ...
      return new Promise(function (resolve, reject) {
        let callback = function (keyImagesStr) {
          ...
          let keyImages = [];
          for (let keyImageJson of JSON.parse(GenUtils.stringifyBIs(keyImagesStr)).keyImages)
    ...
    return that._module.queueTask(async function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        that._module.import_key_images(
          that._cppAddress,
          JSON.stringify({ keyImages: keyImages.map((keyImage) => keyImage.toJson()) }),
          callback,
        );
      });
    });
      ...
      return new Promise(function (resolve, reject) {
        ...
        let callbackFn = function (txSetsJson) {
          if (txSetsJson.charAt(0) !== "{") reject(new MoneroError(txSetsJson)); // json expected, else error
            ...
            let txSets = [];
            for (let txSetJson of JSON.parse(GenUtils.stringifyBIs(txSetsJson)).txSets)
    ...
    return that._module.queueTask(async function () {
      ...
      try {
        return new MoneroTxSet(
          JSON.parse(
            GenUtils.stringifyBIs(that._module.describe_tx_set(that._cppAddress, JSON.stringify(txSet.toJson()))),
          ),
        );
      } catch (err) {
    ...
    return that._module.queueTask(async function () {
      ...
      try {
        return that._module.sign_message(
          that._cppAddress,
          message,
          signatureType === MoneroMessageSignatureType.SIGN_WITH_SPEND_KEY ? 0 : 1,
          accountIdx,
          subaddressIdx,
        );
      } catch (err) {
  ...
  async getAddressBookEntries(entryIndices) {
    ...
    return that._module.queueTask(async function () {
      ...
      for (let entryJson of JSON.parse(
        that._module.get_address_book_entries(that._cppAddress, JSON.stringify({ entryIndices: entryIndices })),
      ).entries) {
  ...
  async getAccountTags() {
    ...
    return that._module.queueTask(async function () {
      that._assertNotClosed();
      let accountTags = [];
      for (let accountTagJson of JSON.parse(that._module.get_account_tags(that._cppAddress)).accountTags)
    ...
    return that._module.queueTask(async function () {
      ...
      try {
        return new MoneroTxConfig(
          JSON.parse(GenUtils.stringifyBIs(that._module.parse_payment_uri(that._cppAddress, uri))),
          true,
  ...
  async startMining(numThreads, backgroundMining, ignoreBattery) {
    ...
    let daemon = new MoneroDaemonRpc(
      Object.assign((await this.getDaemonConnection()).getConfig(), { proxyToWorker: false }),
    );
    ...
    return that._module.queueTask(async function () {
      ...
      return new Promise(function (resolve, reject) {
        that._module.make_multisig(
          that._cppAddress,
          JSON.stringify({ multisigHexes: multisigHexes, threshold: threshold, password: password }),
          (resp) => {
            let errorKey = "error: ";
      ...
      return new Promise(function (resolve, reject) {
        ...
        let callbackFn = async function (cppAddress) {
          if (typeof cppAddress === "string") reject(new MoneroError(cppAddress));
            ...
            resolve(new MoneroWalletFull(cppAddress, path, password, fs, rejectUnauthorized, rejectUnauthorizedFnId));
        };
  ...
  async _backgroundSync() {
    ...
    try {
      ...
      if (!this._isClosed) console.error("Failed to background synchronize " + label + ": " + err.message);
    }
  }
    ...
    return that._module.queueTask(async function () {
      return new Promise(function (resolve, reject) {
        that._module.set_listener(
          ...
          isEnabled
            ? async function (height, startHeight, endHeight, percentDone, message) {
                await that._fullListener.onSyncProgress(height, startHeight, endHeight, percentDone, message);
              }
            : undefined,
          isEnabled
            ...
            : undefined,
          isEnabled
            ? async function (newBalanceStr, newUnlockedBalanceStr, assetType) {
                await that._fullListener.onBalancesChanged(newBalanceStr, newUnlockedBalanceStr, assetType);
              }
            : undefined,
          isEnabled
            ? async function (
    ...
    if (blocksJson.blocks)
      for (let blockJson of blocksJson.blocks)
        deserializedBlocks.blocks.push(
          MoneroWalletFull._sanitizeBlock(new MoneroBlock(blockJson, MoneroBlock.DeserializationType.TX_WALLET)),
        );
    if (blocksJson.missingTxHashes)
      for (let missingTxHash of blocksJson.missingTxHashes) deserializedBlocks.missingTxHashes.push(missingTxHash);
  ...
  static _deserializeTxs(query, blocksJsonStr, missingTxHashes) {
    ...
    if (missingTxHashes === undefined && deserializedBlocks.missingTxHashes.length > 0)
...
class MoneroWalletFullProxy extends MoneroWallet {
  ...
  static async openWalletData(path, password, networkType, keysData, cacheData, daemonUriOrConnection, fs) {
    ...
    let daemonUriOrConfig =
      daemonUriOrConnection instanceof MoneroRpcConnection ? daemonUriOrConnection.getConfig() : daemonUriOrConnection;
    await LibraryUtils.invokeWorker(walletId, "openWalletData", [
      path,
      password,
      networkType,
      keysData,
  ...
  async addListener(listener) {
    ...
    LibraryUtils.WORKER_OBJECTS[this._walletId].callbacks["onSyncProgress_" + listenerId] = [
      wrappedListener.onSyncProgress,
      wrappedListener,
    ];
    LibraryUtils.WORKER_OBJECTS[this._walletId].callbacks["onNewBlock_" + listenerId] = [
      wrappedListener.onNewBlock,
      wrappedListener,
    ];
    LibraryUtils.WORKER_OBJECTS[this._walletId].callbacks["onBalancesChanged_" + listenerId] = [
      wrappedListener.onBalancesChanged,
      wrappedListener,
    ];
    LibraryUtils.WORKER_OBJECTS[this._walletId].callbacks["onOutputReceived_" + listenerId] = [
      wrappedListener.onOutputReceived,
      wrappedListener,
    ];
    LibraryUtils.WORKER_OBJECTS[this._walletId].callbacks["onOutputSpent_" + listenerId] = [
      wrappedListener.onOutputSpent,
      wrappedListener,
    ];
  ...
  async getReserveInfo() {
    const reserveInfo = await this._invokeWorker("getReserveInfo");
    return new TaboReserveInfo(reserveInfo);
  }

  async getBalance(accountIdx, subaddressIdx, asset_type) {
    let balanceStr = await this._invokeWorker("getBalance", Array.from(arguments));

    return new TaboBalance(await this._invokeWorker("getBalance", Array.from(arguments)));
  }

  async getUnlockedBalance(accountIdx, subaddressIdx, asset_type) {
    let unlockedBalanceStr = await this._invokeWorker("getUnlockedBalance", Array.from(arguments));
    return new TaboBalance(unlockedBalanceStr);
  }
  ...
  async getTxs(query, missingTxHashes) {
    ...
    return MoneroWalletFull._deserializeTxs(
      query,
      JSON.stringify({ blocks: respJson.blocks, missingTxHashes: respJson.missingTxHashes }),
      missingTxHashes,
  ...
  async getReserveProofAccount(accountIdx, amount, message) {
    try {
      return await this._invokeWorker("getReserveProofAccount", [accountIdx, amount.toString(), message]);
    } catch (e) {
...
class WalletFullListener {
  ...
  async onBalancesChanged(newBalanceStr, newUnlockedBalanceStr, asset_type) {
    for (let listener of this._wallet.getListeners()) {
      await listener.onBalancesChanged(
        BigInteger.parse(newBalanceStr),
        BigInteger.parse(newUnlockedBalanceStr),
        asset_type,
      );
    }
  }
